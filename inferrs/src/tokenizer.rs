...
impl Tokenizer {
    ...
    fn apply_chat_template(
        ...
        let prompt = match &self.chat_template {
            ...
            ChatTemplate::Qwen36 => apply_qwen36(messages, tools, enable_thinking)?,
    ...
    pub fn apply_chat_template_and_encode(&self, messages: &[ChatMessage]) -> Result<Vec<u32>> {
        self.apply_chat_template_and_encode_with_tools(messages, None, false)
    }
...
mod tests {
    ...
    fn qwen36_jinja_single_user_matches_template() {
        let msgs = vec![user_msg("Hello!")];
        let prompt = apply_qwen36(&msgs, None, false).expect("template render");
        assert!(prompt.contains("<|im_start|>user\nHello!<|im_end|>"));
        assert!(prompt.ends_with("<|im_start|>assistant\n<think>\n\n</think>\n\n"));
    }
