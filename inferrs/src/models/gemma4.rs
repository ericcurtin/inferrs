...
// ---------------------------------------------------------------------------
// MoE (Mixture of Experts) — Gemma4 26B A4B variant
//
// Architecture reference: `Gemma4TextRouter` and `Gemma4TextExperts` in
// `transformers/models/gemma4/modeling_gemma4.py`.
//
// Each decoder layer with `enable_moe_block = true` runs:
//   1. Shared dense MLP on every token (same as the dense 31B model).
//   2. A learned router that selects `top_k_experts` (8) out of `num_experts` (128).
//   3. A sparse expert dispatch: only the selected expert FFNs run per token.
//   4. The two outputs are combined via three extra RMSNorm layers and added.
// ---------------------------------------------------------------------------

/// Plain RMSNorm without a learned scale parameter.
///
/// Matches `Gemma4RMSNorm(with_scale=False)` used inside `Gemma4TextRouter`.
/// Computation is done in F32 regardless of input dtype (matches HF behaviour).
...
impl Gemma4MoeRouter {
    ...
    fn forward(&self, hidden: &Tensor) -> Result<(Tensor, Tensor)> {
...
impl Gemma4MoeBlock {
    ...
    fn forward(&self, shared_mlp_out: &Tensor, residual: &Tensor) -> Result<Tensor> {
