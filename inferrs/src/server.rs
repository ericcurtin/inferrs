...
pub struct ChatCompletionRequest {
    ...
    /// Enable chain-of-thought thinking for models that support it (e.g. Qwen3.6).
    /// When true the model emits a `<think>…</think>` block before answering;
    /// the block is surfaced in `reasoning_content`.
    #[serde(default)]
...
fn make_sse_stream(
    ...
    async_stream::stream! {
            ...
            if uses_native_tools {
                ...
                if finish_reason.is_some() {
                    ...
                    if !outcome.tool_calls.is_empty() {
                        ...
                        // Discard buffered content chunks — the XML must not leak.
        ...
        // Flush any buffered chunks that were never emitted (e.g. stream closed
        // without an explicit finish_reason token).
        for event in buffered_chunks.drain(..) {
            yield Ok(event);
        }
