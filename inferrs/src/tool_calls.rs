...
pub fn process(content: String, finish_reason: String) -> ToolCallOutcome {
    ...
    if tool_calls.is_empty() {
        return ToolCallOutcome { content, tool_calls, finish_reason };
    }
    ToolCallOutcome {
        content: String::new(),
        tool_calls,
...
fn parse_tool_calls(content: &str) -> Vec<ToolCall> {
    ...
    // Find all `<function=name>` anchors.
    let mut search = content;
    while let Some(func_start) = search.find("<function=") {
        let after_tag = &search[func_start + "<function=".len()..];
        let name_end = after_tag.find(|c: char| c == '>' || c == '\n').unwrap_or(after_tag.len());
        let name = after_tag[..name_end].trim().to_string();
        if name.is_empty() {
            search = &search[func_start + 1..];
            continue;
        }
        ...
        // Advance past this function block.
        let consumed = body_start + body_end;
        if consumed >= content.len() {
            break;
        }
        search = &content[consumed..];
    }
...
fn parse_parameters(body: &str) -> String {
    ...
    while let Some(p_start) = rest.find("<parameter=") {
        ...
        let key_end = after.find(|c: char| c == '>' || c == '\n').unwrap_or(after.len());
        ...
        let val_raw = if val_start < rest.len() { &rest[val_start..] } else { "" };
...
mod tests {
    ...
    fn parses_single_tool_call() {
        ...
        assert!(calls[0].function.arguments.contains("latest Elon Musk news"));
    }
