...
/// Fused Q4K GEMV with gelu_mul input: down_proj(gelu(gate)*up) in one dispatch.
...
#[allow(clippy::too_many_arguments)]
pub fn call_quantized_matmul_mv_q4k_gelumul_f32_bf16o(
    device: &Device,
    ep: impl EncoderProvider,
    kernels: &Kernels,
    (b, m, n, k): (usize, usize, usize, usize),
    ...
    up_f32: &Buffer,
    up_offset: usize,
    rhs: &Buffer,    // Q4K down_proj weight
    rhs_offset: usize,
    dst_offset: usize,
    dst: &Buffer,   // BF16 output
) -> Result<(), MetalKernelError> {
    let ne00 = k as i64;
    let ne01 = n as i64;
    let ne02 = b as i64;
    ...
    let pipeline = kernels.load_pipeline(
        device,
        Source::Quantized,
        "kernel_mul_mv_q4_K_gelumul_f32_bf16o",
    )?;
    let encoder = ep.encoder();
    let encoder: &ComputeCommandEncoder = encoder.as_ref();
    encoder.set_compute_pipeline_state(&pipeline);
    ...
    encoder.use_resource(up_f32, MTLResourceUsage::Read);
    encoder.use_resource(rhs, MTLResourceUsage::Read);
    encoder.use_resource(dst, MTLResourceUsage::Write);
    encoder.dispatch_thread_groups(thread_groups_count, threads_per_threadgroup);
    Ok(())
