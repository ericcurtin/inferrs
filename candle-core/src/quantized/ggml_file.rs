...
fn from_raw_data_no_copy<T: super::GgmlType + Send + Sync + 'static>(
    ...
    let storage: QStorage = match device {
        ...
        #[cfg(feature = "metal")]
        // SAFETY: `data` is derived from a memory-mapped file (`raw_data`) whose
        // lifetime is tied to the mmap handle owned by the caller.  The no-copy
        // Metal buffer must not outlive that handle, which the caller guarantees.
        Device::Metal(metal) => unsafe { super::metal::load_quantized_no_copy(metal, data)? },
...
pub fn qtensor_from_ggml_no_copy(
    ...
    match ggml_dtype {
        ...
        GgmlDType::TQ2_0 => {
            from_raw_data_no_copy::<super::BlockTq2_0>(raw_data, size_in_bytes, dims, device)
        }
    }
}
