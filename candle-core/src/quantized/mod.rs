...
impl QTensor {
    ...
    /// Eliminates the BF16→F32 cast before SAXPY. Returns `true` on success.
    #[cfg(feature = "metal")]
    ...
    /// Q8_0 BF16-input SAXPY: result_bf16[i] += w * Q8_0_GEMV(self, xs_bf16)[i].
    ///
    /// Saves 2 dispatches per MoE expert for E2B (Q8_0 weights) vs standard path.
    #[cfg(feature = "metal")]
    pub fn fwd_mv_q8_0_bf16i_saxpy_bf16(
        &self,
        xs: &Tensor,
        result: &Tensor,
        w: f32,
        ...
        match &self.storage {
            QStorage::Metal(m) => {
                m.fwd_mv_q8_0_bf16i_saxpy_bf16(
                    &self.shape,
                    &xs_storage,
                    &xs_layout,
                    &result_storage,
    ...
    /// Q8_0 GEMV: BF16 input → F32 output. For PLI gate and similar F32-output paths.
    pub fn fwd_mv_q8_0_bf16i_f32(&self, xs: &Tensor) -> Result<Option<Tensor>> {
        if xs.dtype() != DType::BF16 {
            return Ok(None);
        }
        let xs_g = xs.storage();
        ...
        match &self.storage {
            QStorage::Metal(m) => {
                ...
                let (ds, dsh) = m.fwd_mv_q8_0_bf16i_f32(&self.shape, &xs_s, &xs_l)?;
                Ok(Some(crate::tensor::from_storage(
                    Storage::Metal(ds),
                    dsh,
                    crate::op::BackpropOp::none(),
    ...
    pub fn fwd_mv_q6k_bf16i_f32(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let (xs_s, xs_l) = match &*xs_g {
            Storage::Metal(s) => (s.clone(), xs.layout().clone()),
            _ => crate::bail!("fwd_mv_q6k_bf16i_f32: Metal storage required"),
        };
        match &self.storage {
            QStorage::Metal(m) => {
                let (ds, dsh) = m.fwd_mv_q6k_bf16i_f32(&self.shape, &xs_s, &xs_l)?;
                Ok(crate::tensor::from_storage(
                    Storage::Metal(ds),
                    dsh,
                    crate::op::BackpropOp::none(),
            ...
            _ => crate::bail!("fwd_mv_q6k_bf16i_f32: Metal QStorage required"),
        }
    }
