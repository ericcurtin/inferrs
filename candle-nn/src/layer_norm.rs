...
impl RmsNorm {
    ...
    pub fn forward_add_then_bf16_out_prealloc(
        ...
        #[cfg(feature = "metal")]
        {
            ...
            let bf16_norm_buf =
                match device.new_buffer(elem_count, DType::BF16, "rmsnorm_add_bf16_then_rmsnorm_bf16_norm") {
                    Ok(b) => b,
                    Err(e) => return Some(Err(e)),
                };
            ...
            match result {
                Ok(()) => {
                    ...
                    let bf16_norm_storage = candle::MetalStorage::new(
                        bf16_norm_buf,
                        device.clone(),
                        elem_count,
                        DType::BF16,
                    ...
                    let bf16_norm = candle::Tensor::from_storage(
                        Storage::Metal(bf16_norm_storage),
                        src_l.shape().clone(),
                        candle::op::BackpropOp::none(),
                        false,
                    );
                    Some(Ok((bf16_out, bf16_norm)))
                }
                Err(e) => Some(Err(candle::Error::wrap(e))),
            }
        }
    ...
    /// Returns `true` when the prealloc path was used; `false` means the caller
    /// must fall back to `forward_add`.
    #[cfg(feature = "metal")]
    pub fn forward_add_prealloc(&self, xs: &Tensor, residual: &Tensor, out: &Tensor) -> bool {
        use candle::{DType, Storage};
        if xs.dtype() != DType::BF16
            || residual.dtype() != DType::BF16
            || out.dtype() != DType::BF16
