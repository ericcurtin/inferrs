...
impl QLinear {
    ...
    #[cfg(feature = "metal")]
    pub fn forward_q4k_bf16i_gelu_mul_f32_prealloc(
        &self,
        other: &QLinear,
        xs_bf16: &Tensor,
        out: &Tensor,
        ...
        qt_s.fwd_mv2_q4k_bf16i_gelu_mul_f32_prealloc(qt_o, xs_bf16, out)
            .unwrap_or(false)
    }
    ...
    /// Q4K GEMV (F32 input) + in-place scaled accumulate into pre-allocated BF16 buffer.
    ///
    /// Computes: `result_bf16[i] += w * GEMV(self, xs_f32)[i]` in one Metal dispatch.
    /// Saves 2 dispatches vs: down_proj GEMV + w-scale + accumulate.
    /// For MoE decode: eliminates 2 × num_active_experts dispatches per MoE layer.
    #[cfg(feature = "metal")]
    #[allow(dead_code)]
    pub fn forward_q4k_f32_saxpy_bf16(&self, xs_f32: &Tensor, result: &Tensor, w: f32) -> bool {
        if self.bias.is_some() {
            return false;
        }
        if xs_f32.dtype() != candle_core::DType::F32 {
        ...
        if !matches!(xs_f32.device(), candle_core::Device::Metal(_)) {
            return false;
        }
        let qt = match &self.inner {
            QMatMul::QTensor(q) => q,
        ...
        qt.fwd_mv_q4k_f32_saxpy_bf16(xs_f32, result, w).unwrap_or(false)
    }

    /// Q4K GEMV (BF16 input) + in-place scaled accumulate into pre-allocated BF16 buffer.
    ///
    /// Computes: `result_bf16[i] += w * GEMV(self, xs_bf16)[i]` in one Metal dispatch.
    /// Eliminates the BF16→F32 cast that `forward_q4k_f32_saxpy_bf16` requires.
    /// For MoE decode: eliminates 1 + 2 = 3 dispatches per expert per MoE layer.
    #[cfg(feature = "metal")]
    pub fn forward_q4k_bf16i_saxpy_bf16(&self, xs_bf16: &Tensor, result: &Tensor, w: f32) -> bool {
        if self.bias.is_some() {
            return false;
        }
        if xs_bf16.dtype() != candle_core::DType::BF16 {
